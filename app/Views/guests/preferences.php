...
<body>
                                ...
                                <div class="form-section">
                                    ...
                                    <div class="mb-3">
                                        <textarea class="form-control" id="question_<?= $question['id'] ?>"
                                            name="questions[<?= $question['id'] ?>]" rows="3"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="followup-field ms-4 mt-2"
                                            id="followup_<?= $question['id'] ?>_<?= $option['id'] ?>"
                                            style="display: none;">
                                            <label
                                                class="form-label text-muted"><?= esc($option['followup_label']) ?></label>
    ...
    function checkPageCompletionAndRedirect() {
        ...
        const requiredInputs = document.querySelectorAll(
            '#questionsContainer input[required], #questionsContainer textarea[required]');
        ...
        const visibleFollowupInputs = document.querySelectorAll(
            '#questionsContainer .followup-field[style*="block"] input[required]');
    ...
    function saveCurrentPageResponses() {
        ...
        document.querySelectorAll(
                '#questionsContainer input[name^="questions["], #questionsContainer textarea[name^="questions["]')
            .forEach(input => {
                ...
                value.forEach(val => {
                    if (val && val.trim() !== '') {
                        const checkbox = document.querySelector(
                            `input[name="questions[${questionId}][]"][value="${val}"]`);
                        if (checkbox) {
                ...
                for (let followupKey in allResponses.followups) {
                    ...
                    if (followupValue && followupValue.trim() !== '') {
                        ...
                        if (!followupField && followupKey.includes('_')) {
                            ...
                            console.log(
                                `Trying to map old format: question ${questionId}, index/option ${indexOrOptionId}`
                                );
                            ...
                            console.log(
                                `Found ${questionFollowupFields.length} follow-up fields for question ${questionId}:`,
                        ...
                        if (followupField && followupInput) {
                            console.log(
                                `✅ Restoring follow-up field ${followupKey} with value: "${followupValue}"`);
                            ...
                            const triggerElement = document.querySelector(
                                `input[data-followup-target="${followupFieldId}"]:checked`);
                            console.log(`Trigger element:`, triggerElement);

                            if (triggerElement && triggerElement.getAttribute('data-followup-required') ===
                                '1') {
                            ...
                            allFollowupFields.forEach(field => {
                                console.log(
                                    `  - ID: "${field.id}", Input name: "${field.querySelector('input')?.name}"`
                                    );
                            });
                        }
                    }
    ...
    function validateCurrentPage() {
        ...
        const textInputs = document.querySelectorAll(
            '#questionsContainer input[required]:not([type="radio"]):not([type="checkbox"]), #questionsContainer textarea[required]'
            );
        ...
        requiredCheckboxGroups.forEach(questionId => {
            ...
            console.log(
            `Checkbox group ${questionId}: ${checkedBoxes.length}/${allCheckboxes.length} selected`);
        ...
        requiredRadioGroups.forEach(groupName => {
            ...
            const checkedRadio = document.querySelector(`input[name="${groupName}"]:checked`);
            console.log(
                `Radio group ${groupName}: ${checkedRadio ? 'selected' : 'none selected'} (${allRadios.length} total)`
                );
        ...
        allFollowupInputs.forEach((input, index) => {
            ...
            if (followupField) {
                ...
                const triggerElement = document.querySelector(
                    `input[data-followup-target="${targetId}"]:checked`);
                const value = input.value ? input.value.trim() : '';

                console.log(
                    `Follow-up ${index + 1} (${targetId}): trigger=${!!triggerElement}, value="${value}"`);
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.followupPreferences) {
                        ...
                        console.log('Merged followup preferences while preserving current session:', allResponses
                            .followups);
                    }
        ...
        if (questions && questions.length > 0) {
            questions.forEach(question => {
                ...
                switch (question.type) {
                    ...
                    default:
                        html +=
                            `<div class="mb-3"><p class="text-warning">Unknown question type: ${escapeHtml(question.type)}</p></div>`;
                        break;
                }
            ...
            html =
            '<div class="text-center py-4"><p class="text-muted">No questions available for this page.</p></div>';
        }
                    ...
                    if (e.target.type === 'radio') {
                        ...
                        if (questionId) {
                            ...
                            if (checkedBoxes.length > 0) {
                                ...
                                document.querySelectorAll(`input[data-question-id="${questionId}"]`).forEach(
                                    checkbox => {
