...
<html lang="en">
<head>
    <title>Register - Islanders App</title>
    <meta charset="utf-8" />
    <meta name="description" content="Register for Islanders App - Streamlining Island Life with Seamless Digital Solutions" />
    <meta name="keywords" content="islanders, register, signup, island, digital solutions" />
    <meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover" />
    <meta name="theme-color" content="#1f2129">
    <meta property="og:locale" content="en_US" />
    <meta property="og:type" content="website" />
    <meta property="og:title" content="Register - Islanders App" />
    <meta property="og:description" content="Join our island community and access digital resources" />
            ...
            <div class="d-flex flex-column flex-lg-row-fluid py-10">
                <div class="d-flex flex-center flex-column flex-column-fluid">
                    <div class="w-lg-500px p-10 p-lg-15 mx-auto">
                        <form class="form w-100" novalidate="novalidate" action="<?= url_to('register') ?>" method="post">
                            <?= csrf_field() ?>
                            <div class="text-center mb-10">
                                <h1 class="text-gray-900 mb-3">Join Islanders App</h1>
                                <div class="text-gray-500 fw-semibold fs-4">
                                    Already have an account?
                                    <a href="<?= url_to('login') ?>" class="link-primary fw-bold">Sign In</a>
                                </div>
                            </div>

                            <div class="fv-row mb-10">
                                <label class="form-label fs-6 fw-bold text-gray-900"><?= lang('Auth.email') ?></label>
                                <input class="form-control form-control-lg form-control-solid <?php if (session('errors.email')) : ?>is-invalid<?php endif ?>"
                                    type="email" name="email" placeholder="<?= lang('Auth.email') ?>" value="<?= old('email') ?>" />
                                <div class="invalid-feedback"><?= session('errors.email') ?></div>
                            </div>

                            <div class="fv-row mb-10">
                                <label class="form-label fs-6 fw-bold text-gray-900"><?= lang('Auth.username') ?></label>
                                <input class="form-control form-control-lg form-control-solid <?php if (session('errors.username')) : ?>is-invalid<?php endif ?>"
                                    type="text" name="username" placeholder="<?= lang('Auth.username') ?>" value="<?= old('username') ?>" />
                                <div class="invalid-feedback"><?= session('errors.username') ?></div>
                            </div>

                            <div class="fv-row mb-10">
                                <label class="form-label fs-6 fw-bold text-gray-900"><?= lang('Auth.password') ?></label>
                                <input class="form-control form-control-lg form-control-solid <?php if (session('errors.password')) : ?>is-invalid<?php endif ?>"
                                    placeholder="<?= lang('Auth.password') ?>" type="password" name="password" autocomplete="off" />
                                <div class="invalid-feedback"><?= session('errors.password') ?></div>
                            </div>

                            <div class="fv-row mb-10">
                                <label class="form-label fs-6 fw-bold text-gray-900"><?= lang('Auth.repeatPassword') ?></label>
                                <input class="form-control form-control-lg form-control-solid <?php if (session('errors.pass_confirm')) : ?>is-invalid<?php endif ?>"
                                    placeholder="<?= lang('Auth.repeatPassword') ?>" type="password" name="pass_confirm" autocomplete="off" />
                                <div class="invalid-feedback"><?= session('errors.pass_confirm') ?></div>
                            </div>

                            <div class="form-check mb-10">
                                <label class="form-check-label">
                                    <input type="checkbox" name="terms" class="form-check-input" value="1" required>
                                    I Accept the <a href="#" class="link-primary fw-bold">Terms & Conditions</a>
                                </label>
                            </div>

                            <div class="text-center">
                                <button type="submit" class="btn btn-lg btn-dark w-100 mb-5"><?= lang('Auth.register') ?></button>
                            </div>
                        </form>
                    </div>
                </div>
