...
<div class="d-lg-none">
    ...
    <div class="mobile-search-bar position-sticky top-0 py-3 mb-2" style="top: 60px !important;">
        <div class="container-fluid">
            <div class="mb-2">
                <h1 class="text-dark fw-bold ms-2">Divisions</h1>
            </div>
                ...
                <div class="col-10">
                    <div class="position-relative h-100">
                        ...
                        <input type="text" id="mobile_search" class="form-control form-control-solid ps-10 h-100"
                            placeholder="Search divisions..." value="<?= esc($search) ?>" />
                    </div>
                </div>
                <div class="col-2">
                    <?php if ($permissions['canCreate']): ?>
                    <button type="button" data-bs-toggle="modal" data-bs-target="#createDivisionModal"
                    ...
                    <div class="btn btn-light-secondary w-100 h-100 d-flex align-items-center justify-content-center disabled"
                        style="min-height: 48px;" title="No permission to create divisions">
    ...
    <div class="container-fluid" style="padding-top: 5px;">
        ...
        <div class="row mt-2" id="mobile-cards-container">
            ...
            <div class="col-12 mb-3" data-aos="fade-up" data-aos-delay="<?= $index * 100 ?>" data-aos-duration="600">
                <div class="card mobile-division-card" data-division-id="<?= esc($division['id']) ?>">
                    <div class="card-body p-4">
                        <!-- Division Header -->
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div class="flex-grow-1">
                                <small class="text-muted text-uppercase">#<?= esc($division['id']) ?></small>
                            </div>
                            <div class="ms-3 d-flex gap-2">
                                <?php if (!empty($division['status_name'])): ?>
                                    <?php 
                                    // Use custom color if available for mobile cards
                                    if (!empty($division['status_color'])) {
                                        // Convert hex color to RGB for light background
                                        $hex = ltrim($division['status_color'], '#');
                                        $r = hexdec(substr($hex, 0, 2));
                                        $g = hexdec(substr($hex, 2, 2));
                                        $b = hexdec(substr($hex, 4, 2));
                                        $lightBg = "rgba($r, $g, $b, 0.1)";
                                        $textColor = $division['status_color'];
                                        $mobileBadgeStyle = "background-color: $lightBg; color: $textColor;";
                                    } else {
                                        $mobileBadgeStyle = "";
                                    }
                                    ?>
                                    <?php if (!empty($division['status_color'])): ?>
                                    <span class="badge fw-bold fs-8" style="<?= $mobileBadgeStyle ?>">
                                        <?= strtoupper(esc($division['status_name'])) ?>
                                    </span>
                                    <?php else: ?>
                                    ...
                                    <?php endif; ?>
                                <?php endif; ?>
                                <span class="badge badge-light-primary fw-bold">DIVISION</span>
                            </div>
                        </div>

                        <div class="d-flex justify-content-between align-items-start mb-4 mt-4">
                            <div class="flex-grow-1">
                                <strong class="me-5 text-uppercase text-truncate"><?= esc($division['name']) ?></strong>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mt-4">
                            <div class="d-flex flex-column">
                                <small class="text-muted">
                                    <?= !empty($division['created_by_name']) ? esc($division['created_by_name']) : 'System' ?>
                                </small>
                            </div>
                            <small class="text-muted"><?= date('M d, Y', strtotime($division['created_at'])) ?></small>
                        </div>
                        ...
                        <div class="mobile-actions mt-3 pt-3 border-top d-none">
                            <div class="row g-2">
                                ...
                                <div class="col-4">
                                    <button type="button"
                                        class="btn btn-light-warning btn-sm w-100 d-flex align-items-center justify-content-center view-division-btn"
                                        data-division-id="<?= esc($division['id']) ?>">
                                ...
                                <div class="col-4">
                                    <button type="button"
                                        class="btn btn-light-primary btn-sm w-100 d-flex align-items-center justify-content-center edit-division-btn"
                                        data-division-id="<?= esc($division['id']) ?>">
                                ...
                                <div class="col-4">
                                    <button
                                        class="btn btn-light-danger btn-sm w-100 d-flex align-items-center justify-content-center delete-division-btn"
                                        data-division-id="<?= esc($division['id']) ?>">
            ...
            <div class="col-12">
                <div class="d-flex flex-column align-items-center justify-content-center py-10">
                    ...
                    <h6 class="fw-bold text-gray-700 mb-2">No divisions found</h6>
                    <p class="fs-7 text-gray-500 mb-4">Start by creating your first division entry</p>
                </div>
            </div>
            <?php endif; ?>
        </div>
        ...
        <div id="loading-indicator" class="text-center py-4 d-none">
            ...
            <p class="mt-2 text-muted">Loading more divisions...</p>
        </div>

        <!-- No more data indicator -->
        <div id="no-more-data" class="text-center py-4 d-none">
            <p class="text-muted">No more divisions to load</p>
        </div>
    </div>
</div>
<!--end::Mobile UI-->
                ...
                <div class="card">
                    ...
                    <div class="card-header border-0 pt-6">
                        ...
                        <div class="card-title">
                            ...
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <input type="text" id="kt_filter_search"
                                    class="form-control form-control-solid w-250px ps-13" placeholder="Search divisions..."
                                    value="<?= esc($search) ?>" />
                            </div>
                            <!--end::Search-->
                        </div>
                        ...
                        <div class="card-toolbar">
                            <!--begin::Toolbar-->
                            <div class="d-flex justify-content-end" data-kt-division-table-toolbar="base">
                                <!--begin::Add division-->
                                <?php if ($permissions['canCreate']): ?>
                                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                    data-bs-target="#createDivisionModal">
                                    <i class="ki-duotone ki-plus fs-2"></i>Add Division
                                </button>
                                <?php endif; ?>
                                <!--end::Add division-->
                            </div>
                    ...
                    <div class="card-body py-4">
                        ...
                        <div class="table-responsive">
                            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_division_table">
                                <!--begin::Table head-->
                                <thead>
                                    <!--begin::Table row-->
                                        ...
                                        <th class="w-10px pe-2">
                                            <div
                                                ...
                                                <input class="form-check-input" type="checkbox" data-kt-check="true"
                                                    data-kt-check-target="#kt_division_table .form-check-input"
                                                    value="1" />
                                            </div>
                                        </th>
                                ...
                                <tbody class="text-gray-600 fw-semibold">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex flex-column">
                                                <small class="text-muted">#<?= esc($division['id']) ?></small>
                                            </div>
                                        </td>
                                        <!--end::ID-->
                                        ...
                                        <td>
                                            <div class="d-flex flex-column">
                                                <?php if (!empty($division['created_by_name'])): ?>
                                                <span class="text-muted"><?= esc($division['created_by_name']) ?></span>
                                                <small
                                                    class="text-muted"><?= date('d M Y \a\t H:i', strtotime($division['created_at'])) ?></small>
                                                <?php endif; ?>
                                            </div>
                                        </td>
                                        <!--end::Created By-->
                                        ...
                                        <td>
                                            <div class="d-flex flex-column">
                                                <?php if (!empty($division['updated_by_name']) && !empty($division['updated_at'])): ?>
                                                <span class="text-muted"><?= esc($division['updated_by_name']) ?></span>
                                                <small
                                                    class="text-muted"><?= date('d M Y \a\t H:i', strtotime($division['updated_at'])) ?></small>
                                                <?php endif; ?>
                                            </div>
                                        </td>
                                        <!--end::Updated By-->
                                        ...
                                        <td class="text-end">
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                                ...
                                                <div class="menu-item px-3">
                                                    <a class="menu-link px-3 view-division-btn"
                                                        data-division-id="<?= esc($division['id']) ?>">View</a>
                                                </div>
                                                <?php endif; ?>
                                                <!--end::Menu item-->
                                                <!--begin::Menu item-->
                                                <?php if ($permissions['canEdit']): ?>
                                                <div class="menu-item px-3">
                                                    <a class="menu-link px-3 edit-division-btn"
                                                        data-division-id="<?= esc($division['id']) ?>">Edit</a>
                                                </div>
                                                <?php endif; ?>
                                                <!--end::Menu item-->
                                                <!--begin::Menu item-->
                                                <?php if ($permissions['canDelete']): ?>
                                                <div class="menu-item px-3">
                                                    <a class="menu-link px-3 delete-division-btn"
                                                        data-division-id="<?= esc($division['id']) ?>">Delete</a>
                                                </div>
                                                <?php endif; ?>
                                                <!--end::Menu item-->
                                            </div>
                                    ...
                                    <tr>
                                        <td colspan="7" class="text-center py-10">
                                            <div class="d-flex flex-column align-items-center">
                                                ...
                                                <div class="fw-bold text-gray-700 mb-2">No divisions found</div>
                                                <div class="text-gray-500">Start by creating your first division entry
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
...
<!-- Include Modals -->
<?= $this->include('divisions/create_modal') ?>
<?= $this->include('divisions/edit_modal') ?>
<?= $this->include('divisions/view_modal') ?>
...
document.addEventListener('DOMContentLoaded', function() {
    const existingCards = document.querySelectorAll('#mobile-cards-container .mobile-division-card');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load initial divisions for mobile
    const mobileContainer = document.getElementById('mobile-cards-container');
    if (mobileContainer) {
        // Only load initial data if container is empty (no server-rendered content)
        const existingCards = mobileContainer.querySelectorAll('.mobile-division-card');
        if (existingCards.length === 0) {
            loadDivisions(false);
        }
            ...
            scrollTimeout = setTimeout(() => {
                if (!isLoading && hasMoreData) {
                    ...
                    if (scrollPosition >= documentHeight - 1000) {
                        loadDivisions(false, mobileSearch?.value || '');
                    }
                }
            }, 100);
        });
    ...
    // Handle view division
    document.addEventListener('click', function(e) {
        if (e.target.closest('.view-division-btn')) {
            e.preventDefault();
            const divisionId = e.target.closest('.view-division-btn').getAttribute('data-division-id');
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.edit-division-btn')) {
            e.preventDefault();
            const divisionId = e.target.closest('.edit-division-btn').getAttribute('data-division-id');
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.delete-division-btn')) {
            e.preventDefault();
            const divisionId = e.target.closest('.delete-division-btn').getAttribute('data-division-id');
...
function loadDivisions(reset = false, search = '') {
    if (isLoading) return;
    ...
    const url = `/divisions/api?page=${currentPage}&limit=10&search=${encodeURIComponent(search)}`;
...
function renderDivisions(divisions) {
    ...
    divisions.forEach((division, index) => {
        const divisionCard = createDivisionCard(division, (currentPage - 1) * 10 + index);
...
function createDivisionCard(division, index) {
    ...
    const description = division.description ?
        `<p class="text-muted mb-0 mt-3">${division.description}</p>` : '';

    const createdByName = division.created_by_name || 'System';

    const createdAt = new Date(division.created_at).toLocaleDateString('en-US', {
        month: 'short',
        day: '2-digit',
        year: 'numeric'
    });
    ...
    actionButtons += `
        <div class="col-4">
            <button type="button" class="btn btn-light-warning btn-sm w-100 d-flex align-items-center justify-content-center view-division-btn" data-division-id="${division.id}">
                <i class="ki-duotone ki-eye fs-1 me-2">
                    <span class="path1"></span>
                    <span class="path2"></span>
                    <span class="path3"></span>
    ...
    actionButtons += `
        <div class="col-4">
            <button type="button" class="btn btn-light-primary btn-sm w-100 d-flex align-items-center justify-content-center edit-division-btn" data-division-id="${division.id}">
                <i class="ki-duotone ki-pencil fs-1 me-2">
                    <span class="path1"></span>
                    <span class="path2"></span>
                </i>
    ...
    actionButtons += `
        <div class="col-4">
            <button class="btn btn-light-danger btn-sm w-100 d-flex align-items-center justify-content-center delete-division-btn" data-division-id="${division.id}">
                <i class="ki-duotone ki-trash fs-1 me-2">
                    <span class="path1"></span>
                    <span class="path2"></span>
                    <span class="path3"></span>
    ...
    col.innerHTML = `
        <div class="card mobile-division-card" data-division-id="${division.id}">
            <div class="card-body p-4">
                <!-- Division Header -->
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div class="flex-grow-1">
                        <small class="text-muted text-uppercase">#${division.id}</small>
                    </div>
                    <div class="ms-3 d-flex gap-2">
                        ${division.status_name ? `
                            ${division.status_color ? 
                                `<span class="badge fw-bold fs-8" style="background-color: ${division.status_color}1a; color: ${division.status_color};">${division.status_name.toUpperCase()}</span>` :
                                `<span class="badge badge-light-success fw-bold fs-8">${division.status_name.toUpperCase()}</span>`
                            }
                        ` : ''}
                        <span class="badge badge-light-primary fw-bold">DIVISION</span>
                    </div>
                ...
                <div class="d-flex justify-content-between align-items-start mb-4 mt-4">
                    <div class="flex-grow-1">
                        <strong class="me-5 text-uppercase text-truncate">${division.name}</strong>
                    </div>
                </div>
...
function showNoDivisionsMessage() {
    const container = document.getElementById('mobile-cards-container');
    if (!container) return;
...
function viewDivision(divisionId) {
    secureFetch(`/divisions/show/${divisionId}`)
        .then(response => {
            if (response.status === 401 || response.status === 403) {
                handleSessionExpired();
                return;
        ...
        .then(data => {
            if (data.success) {
                ...
                // Show modal
                const modal = new bootstrap.Modal(document.getElementById('viewDivisionModal'));
                modal.show();
            } else {
...
function deleteDivision(divisionId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            secureFetch(`/divisions/delete/${divisionId}`, {
                    method: 'POST',
                    headers: {
...
function initMobileCards() {
    ...
    document.querySelectorAll('.mobile-division-card').forEach(function(card) {
...
function populateViewModal(division) {
    // Basic division info
    document.getElementById('view_division_name').textContent = division.name;
    ...
    if (division.description && division.description.trim() !== '') {
        description.textContent = division.description;
    ...
    // Audit info
    document.getElementById('view_created_by').textContent = division.created_by_name || 'System';
    document.getElementById('view_created_at').textContent = division.created_at ? 
        new Date(division.created_at).toLocaleDateString('en-US', { 
            year: 'numeric', 
            month: 'long', 
            day: 'numeric',
            hour: '2-digit',
    ...
    if (division.updated_by_name) {
        document.getElementById('view_updated_by').textContent = division.updated_by_name;
    ...
    if (division.updated_at) {
        document.getElementById('view_updated_at').textContent = new Date(division.updated_at).toLocaleDateString('en-US', { 
            year: 'numeric', 
            month: 'long', 
            day: 'numeric',
            hour: '2-digit',
    ...
    // Set division ID for edit button if it exists
    const editBtn = document.getElementById('view_edit_btn');
    if (editBtn) {
        editBtn.setAttribute('data-division-id', division.id);
    }
}

function populateEditModal(division) {
    document.getElementById('edit_division_id').value = division.id;
    document.querySelector('#editDivisionForm input[name="name"]').value = division.name;
    document.querySelector('#editDivisionForm textarea[name="description"]').value = division.description || '';
    
    // Set status dropdown
    const statusSelect = document.querySelector('#editDivisionForm select[name="status_id"]');
