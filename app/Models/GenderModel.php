...
class GenderModel extends Model
{
    ...
    protected $validationRules = [
        'name' => [
            ...
            'errors' => [
                ...
                'min_length'  => 'Gender name must be at least 3 characters long.',
                'max_length'  => 'Gender name cannot exceed 100 characters.'
            ]
        ],
        'status_id' => [
            'label'  => 'Status',
    ...
    public function getActiveStatuses()
    {
        ...
        return $statusModel->where('module_id', 1)->findAll(); // Module ID 1 for genders
    }

    /**
     * Get gender by ID with validation
     */
    public function getGender($id)
    {
        $builder = $this->db->table('genders g');
        ...
        return $builder->select('g.*, 
                      ...
                      ->join('status s', 's.id = g.status_id', 'left')
                      ->join('users cu', 'cu.id = g.created_by', 'left')
     ...
     */
    public function getGendersWithPagination($search = '', $limit = 10, $offset = 0)
    {
        $builder = $this->db->table('genders g');
    ...
    /**
     * Count genders with search filter
     */
    public function getGendersCount($search = '')
    {
        $builder = $this->db->table('genders g');
        
        // Join with status and users tables for consistent search results
        $builder->join('status s', 's.id = g.status_id', 'left')
                ->join('users cu', 'cu.id = g.created_by', 'left')
                ->join('users uu', 'uu.id = g.updated_by', 'left');
    ...
    public function getUpdateValidationRules($id)
    {
        ...
        $rules['name']['rules'] = "required|min_length[3]|max_length[100]|is_unique[genders.name,id,{$id}]";
        $rules['name']['errors']['is_unique'] = 'This gender name already exists.';
        return $rules;
    }
